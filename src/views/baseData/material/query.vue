...
<script>
import { getICItemRecord, deleteICItem } from "@/api/record";
import { billQueryMixin } from "../mixins";
export default {
  mixins: [billQueryMixin],
  provide: {
    billName: "物料档案" // 获取页面配置信息的具体标识
  },
  methods: {
    query(data) {
      this.queryConditions = data;
      this.queryHandler(getICItemRecord(data));
    },
    deleteRow(fItemId) {
      this.deleteHandler(deleteICItem({ fItemId }));
    }
  }
};
</script>
